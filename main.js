...
function update_rays() {
	...
	const pathObj = [];
	const pathImg = [];
	const posA = new THREE.Vector3(vars.sep / 2, vars.sepy, 0);
	const posB = new THREE.Vector3(-vars.sep / 2, vars.sepy, 0);
	const num = vars.density < 1 ? 1 : vars.sep * vars.density;
	for (let i = 0; i <= num; ++i) {
		...
		const path = genPath(body, callback, 1000, 0.01);
		if (path[path.length - 1].y <= vars.sepy) {
			pathImg.push(path[path.length - 1]);
		}
		{ // Draw can stuff
...

{
	const folder_field = gui.addFolder("Magnetic Field");
		...
		const folder_field_down = folder_field.addFolder("Down Component");
			...
			folder_field_down.add(vars, "x_a", 0, 10).name("Offset / m").onChange(update_all);
			folder_field_down.add(vars, "t_a", 0, 10).name("Std Dev / m").onChange(update_all);
		const folder_field_radial = folder_field.addFolder("Radial Component");
			folder_field_radial.open();
			folder_field_radial.add(vars, "x_b", 0, 10).name("Offset / m").onChange(update_all);
			folder_field_radial.add(vars, "t_b", 0, 10).name("Std Dev / m").onChange(update_all);
	const folder_particles = gui.addFolder("Particles");
		...
		folder_particles.add(vars, "sep", 0.1, 10).name("Horizontal Gap / m").onChange(update_all);
		folder_particles.add(vars, "sepy", 0.1, 20).name("Vertical Gap / m").onChange(update_all);
